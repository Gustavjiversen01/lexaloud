"""GNOME top-bar tray indicator for Lexaloud.
...
try:
    ...
    print(
        f"Lexaloud indicator: missing GIR typelib: {e}. "
...
LEXALOUD_BIN = str(VENV_BIN / "lexaloud")
...
def _systemctl(action: str) -> int:
    """Run `systemctl --user <action> lexaloud.service` with full error
...
class LexaloudIndicator:
    def __init__(self) -> None:
        ...
        self.indicator.set_title("Lexaloud")
    ...
    def _on_speak_selection(self, _src) -> None:
        self._spawn_detached([LEXALOUD_BIN, "speak-selection"])

    def _on_pause_resume(self, _src) -> None:
        self._spawn_detached([LEXALOUD_BIN, "toggle"])
...
def main() -> int:
    """Entry point for `lexaloud-indicator`."""
