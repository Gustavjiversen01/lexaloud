# Installing ReadAloud on Ubuntu 24.04 GNOME
...
- creates a venv at `~/.local/share/readaloud/venv/`
- installs the pinned dependency set from `requirements-lock.txt`
- installs the `readaloud` package (editable) into the venv
...
1. Resolve the absolute path of the `readaloud` binary.
2. Download the Kokoro model artifacts (`kokoro-v1.0.onnx` ~310 MB,
   `voices-v1.0.bin` ~28 MB) into `~/.cache/readaloud/models/` and verify
   their SHA256 hashes.
3. Write a systemd `--user` unit file to
   `~/.config/systemd/user/readaloud.service`. If the file already exists,
...
```bash
systemctl --user status readaloud.service
~/.local/share/readaloud/venv/bin/readaloud status
```

You should see `"state": "idle"` with `"session_providers": [..., "CUDAExecutionProvider", ...]` (or `"CPUExecutionProvider"` alone if CUDA setup failed — in which case check the daemon logs with `journalctl --user -u readaloud`).

## 5. Bind a global hotkey

Open **Settings → Keyboard → View and Customize Shortcuts → Custom Shortcuts → `+` (Add shortcut)** and create:

- **Name:** `ReadAloud: speak selection`
- **Command:** `/home/<you>/.local/share/readaloud/venv/bin/readaloud speak-selection`
- **Shortcut:** press `Super+R` (or any binding you prefer)

(`readaloud setup` printed the exact command path for you — copy that, do not retype.)
...
| Symptom | Check |
|---|---|
| `exit 3: ReadAloud daemon is not running` | `systemctl --user status readaloud.service` and `journalctl --user -u readaloud` |
...
| Voice sounds robotic | You're on a very old Kokoro version OR you fell back to speech-dispatcher+espeak. Check `readaloud status` — the provider name should be `kokoro`. |
| `exit 1: SHA256 mismatch` | A model file is corrupt. Delete `~/.cache/readaloud/models/` and run `readaloud download-models`. |
...
| Daemon refuses to start with "Both onnxruntime and onnxruntime-gpu installed" | Recreate the venv: `rm -rf ~/.local/share/readaloud/venv && ./scripts/install.sh` |
