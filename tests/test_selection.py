...
from readaloud.selection import (
    SelectionEmpty,
    SelectionTimeout,
...
def _patch_env(session: _FakeSession, run_stdout: bytes, run_raises=None):
    return (
        patch("readaloud.selection.detect_session", return_value=session),
        patch(
            "readaloud.selection.shutil.which",
            side_effect=lambda name: "/usr/bin/" + name,
        ),
        patch(
            "readaloud.selection.subprocess.run",
...
def test_read_primary_wayland_returns_text():
    ...
    with patch("readaloud.selection.detect_session", return_value=session), \
         patch("readaloud.selection.shutil.which", return_value="/usr/bin/wl-paste"), \
         patch("readaloud.selection.subprocess.run", return_value=_completed(b"hello world")):
...
def test_read_primary_empty_raises():
    ...
    with patch("readaloud.selection.detect_session", return_value=session), \
         patch("readaloud.selection.shutil.which", return_value="/usr/bin/wl-paste"), \
         patch("readaloud.selection.subprocess.run", return_value=_completed(b"")):
...
def test_read_primary_truncates_large_selection():
    ...
    with patch("readaloud.selection.detect_session", return_value=session), \
         patch("readaloud.selection.shutil.which", return_value="/usr/bin/wl-paste"), \
         patch("readaloud.selection.subprocess.run", return_value=_completed(big_text)):
...
def test_read_primary_tool_missing_raises():
    ...
    with patch("readaloud.selection.detect_session", return_value=session), \
         patch("readaloud.selection.shutil.which", return_value=None):
...
def test_read_primary_timeout_raises():
    ...
    with patch("readaloud.selection.detect_session", return_value=session), \
         patch("readaloud.selection.shutil.which", return_value="/usr/bin/wl-paste"), \
         patch("readaloud.selection.subprocess.run", side_effect=_raise):
...
def test_read_clipboard_x11_uses_xclip():
    ...
    with patch("readaloud.selection.detect_session", return_value=session), \
         patch("readaloud.selection.shutil.which", return_value="/usr/bin/xclip"), \
         patch("readaloud.selection.subprocess.run", return_value=_completed(b"clipped text")) as mock_run:
