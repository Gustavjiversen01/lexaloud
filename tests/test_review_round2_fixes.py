...
from readaloud.audio import NullSink
from readaloud.config import Config, load_config
...
def test_display_unavailable_raised_on_xclip_cant_open_display():
    with patch("readaloud.selection.shutil.which", return_value="/usr/bin/xclip"):
        with patch(
...
def test_display_unavailable_raised_on_wl_paste_no_connection():
    with patch("readaloud.selection.shutil.which", return_value="/usr/bin/wl-paste"):
        with patch(
            "readaloud.selection.subprocess.run",
            return_value=_completed(
...
def test_toml_escape_handles_control_chars():
    from readaloud.gui_control import _toml_escape, _save_config_dict, _load_config_dict
...
def test_toml_save_load_round_trip(tmp_path, monkeypatch):
    from readaloud.gui_control import _save_config_dict, _load_config_dict
...
def test_toml_save_does_not_write_empty_sections(tmp_path, monkeypatch):
    from readaloud.gui_control import _save_config_dict

    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))

    data = {"empty": {}, "provider": {"voice": "af_heart"}}
    _save_config_dict(data)
    content = (tmp_path / "readaloud" / "config.toml").read_text()
